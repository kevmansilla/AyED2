...
unsigned int passengers_amount_in_airport (LayoverTable a, unsigned int h) {
  unsigned int passengers_in_arpt = a[0][arrival].passengers_amount;
  for (unsigned int hour = 1u; hour <= h; ++hour) {
    passengers_in_arpt = passengers_in_arpt + a[hour][arrival].passengers_amount;
  }
  passengers_in_arpt = passengers_in_arpt + a[h][departure].passengers_amount;
  return passengers_in_arpt;
}
